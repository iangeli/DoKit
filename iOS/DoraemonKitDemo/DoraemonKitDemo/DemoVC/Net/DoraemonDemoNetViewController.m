...
- (void)netForAFNetworking2 {
    ...
    [manager POST:@"https://www.taobao.com/"
        parameters:nil
        headers:nil
        progress:nil
        success:^(NSURLSessionDataTask *_Nonnull task, id _Nullable responseObject) {
...
- (void)mockTest {
    [DoraemonNetworkUtil getWithUrlString:@"http://172.23.162.150:8080/api/topics/hot.json?name=yi"
        params:nil
...
- (void)mockTest2 {
    [DoraemonNetworkUtil postWithUrlString:@"http://172.23.162.150:8080/api/topics/hot.json"
        params:@{@"bodyTitle" : @"bodyName"}
        success:^(NSDictionary *_Nonnull result) {
            NSLog(@"result == %@", result);
        }
        error:^(NSError *_Nonnull error) {
