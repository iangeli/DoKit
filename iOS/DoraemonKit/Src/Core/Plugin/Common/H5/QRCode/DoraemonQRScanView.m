...
- (BOOL)statusCheck {
    if (![DoraemonQRScanView isCameraAvailable]) {
        [DoraemonAlertUtil handleAlertActionWithVC:[self doraemon_viewController]
                                              text:@"Device without camera"
                                           okBlock:^{
    ...
    if (![self isCameraAuthStatusCorrect]) {
        [DoraemonAlertUtil handleAlertActionWithVC:[self doraemon_viewController]
            text:@"Camera permission is not open"
            okBlock:^{
