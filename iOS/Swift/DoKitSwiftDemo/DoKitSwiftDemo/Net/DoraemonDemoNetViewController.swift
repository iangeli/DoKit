...
class DoraemonDemoNetViewController: DoraemonDemoBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func netForURLConnection() {
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: OperationQueue()) { (_: URLResponse?, data: Data?, error: Error?) in
            if error == nil {
    ...
    func netForNSURLSession() {
        ...
        let task: URLSessionDataTask = session.dataTask(with: request) { (data: Data?, _: URLResponse?, error: Error?) in
            if error == nil {
                let result: String = String(data: data!, encoding: String.Encoding.utf8)!
                print("请求成功 = \(result)")
            } else {
    ...
    func netForAFNetworking() {
        ...
        manager.get("https://www.taobao.com/", parameters: nil, progress: nil, success: { (_, response) in
